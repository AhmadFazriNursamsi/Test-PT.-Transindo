...
$haveaccessadd = Helpers::checkaccess('size', 'add');
$haveaccessdelete = Helpers::checkaccess('size', 'delete');
?>
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="datastable" class="table text-start table-striped align-middle table-bordered table-hover mb-0">
                            <thead>
                                <tr>
                                    <td></td>
                                    <td><input type="text" class="form-control input-sm src_class_user" autocomplete="off" onkeyup="searcAjax(this)" placeholder="Name Size" name="satuan_name"></td>
                                    <td>
                                ...
                                <tr>
                                    ...
                                    <th class="align-center">Name Size</th>
                        ...
                        <div class="col-md-12">
                            <form id="smbtn" enctype="multipart/form-data"> 
                                ...
                                <div class="form-group">
                                    ...
                                    <label for="satuan_name" class="form-label">Name Size*</label>
                                    <input type="text" class="form-control inpt-cst-add mb-2" name="satuan_name" id="satuan_name" placeholder="Nama Satuan">
                                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    @if ($haveaccessadd) :
                        <span id="editvbtn" data-attid="" class="btn btn-success btn-sm"><i class="fa fa-edit"></i>Edit Size</span>
                    @endif
...
<script type="text/javascript">
    ...
    var url = "{{ asset('/api/size/getdata') }}";
    // console.log(url);
    ...
    $(document).ready(function()
    {
        var table = $('#datastable').DataTable({
            ...
            createdRow: function(row, data, dataIndex, cells) {
                ...
                // console.log(data[1]); // Nama Satuan
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    $("#btnAdd").click(function(){
        ...
        $('#active').val(1).change(); //
        $('#viewad').modal('show'); 
        $("#ModalLongTitle").html("Size Add"); // title MODAL CREATE
        $("#addvbtn").html('<i class="fa fa-plus"></i> Add Size');
    });
    // UNTUK MEMBUKA MODAL ADD
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    $("#smbtn").submit(function(e)
    {
        ...
        var url = "{{ asset('api/size/insertdata') }}";
        if(id != '') 
            var url = "{{ asset('api/size/updatedata') }}/" + id;
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    function showdetail(idx) 
    {
        ...
        $('#deletevbtn').html('<i class="fa fa-trash"></i>Delete Size');

        $('#undeletevbtn').attr('data-attid', idx);
        $('#undeletevbtn').html('<i class="fa fa-repeat"></i> Undelete Size');

        var table3 = document.querySelector("#table_detail tbody");
        table3.innerHTML= "";

        var url = "{{ asset('/api/size/getdatabyid') }}/" + idx; // Table View Details
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    {        
        ...
        $("#ModalLongTitle").html("Edit Satuan");
        clearInput("inpt-cst-add"); // pak heri

        $("#addvbtn").html('<i class="fa fa-edit"></i> Edit Satuan');
        $("#view").modal('hide');
        $('#viewad').modal('show');
        ...
        $.ajax({
            ...
            success: function(response) 
            {
                data = response.data; // success
                $("#id").val(data.id);
                $("#satuan_name").val(data.satuan_name);
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    function deleteyesshow() {
        ...
        Swal.fire({
            ...
            {
                var url = "{{ asset('/api/size/delete') }}/" + idx;
                $.ajax({
                    url: url,
                    type: "POST",
                    data: {
