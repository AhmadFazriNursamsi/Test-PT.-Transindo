...
$haveaccessadd = Helpers::checkaccess('suppliers', 'add');
$haveaccessdelete = Helpers::checkaccess('suppliers', 'delete');
?>
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="datastable" class="table text-start table-striped align-middle table-bordered table-hover mb-0">
                            <thead>
                                <tr>
                                    <td></td>
                                    <td><input type="text" class="form-control input-sm src_class_user" autocomplete="off" onkeyup="searcAjax(this)" placeholder="Nama Supplier" name="supplier_name"></td>
                                    <td>
                                ...
                                <tr>
                                    ...
                                    <th class="align-center">Nama Supplier</th>
                        ...
                        <div class="col-md-12">
                            <form id="smbtn" enctype="multipart/form-data"> 
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control inpt-cst-add mb-2" name="supplier_name" id="supplier_name" placeholder="Nama Supplier">
                                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    @if ($haveaccessadd) :
                        <span id="editvbtn" data-attid="" class="btn btn-success btn-sm"><i class="fa fa-edit"></i>Edit Supplier</span>
                    @endif
...
<script type="text/javascript">
    ...
    var url = "{{ asset('/api/suppliers/getdata') }}";
    // console.log(url);
    ...
    $(document).ready(function()
    {
        var table = $('#datastable').DataTable({
            ...
            createdRow: function(row, data, dataIndex, cells) {
                ...
                // console.log(data[1]); // Nama Supplier
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    $("#btnAdd").click(function(){
        ...
        $('#active').val(1).change(); //
        $('#viewad').modal('show'); 
        $("#ModalLongTitle").html("Supplier Tambah"); // title MODAL CREATE
        $("#addvbtn").html('<i class="fa fa-plus"></i> Add Supplier');
    });
    // UNTUK MEMBUKA MODAL ADD
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    $("#smbtn").submit(function(e)
    {
        ...
        var url = "{{ asset('api/suppliers/insertdata') }}";
        if(id != '') 
            var url = "{{ asset('api/suppliers/updatedata') }}/" + id;
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    function showdetail(idx) 
    {
        ...
        $('#deletevbtn').html('<i class="fa fa-trash"></i>Delete Supplier');

        $('#undeletevbtn').attr('data-attid', idx);
        $('#undeletevbtn').html('<i class="fa fa-repeat"></i> Undelete Supplier');

        var table3 = document.querySelector("#table_detail tbody");
        table3.innerHTML= "";

        var url = "{{ asset('/api/suppliers/getdatabyid') }}/" + idx; // Table View Details
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    {        
        ...
        $("#ModalLongTitle").html("Edit Supplier");
        clearInput("inpt-cst-add"); // pak heri

        $("#addvbtn").html('<i class="fa fa-edit"></i> Edit Supplier');
        $("#view").modal('hide');
        $('#viewad').modal('show');
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    function deleteyesshow() {
        ...
        Swal.fire({
            ...
            {
                var url = "{{ asset('/api/suppliers/delete') }}/" + idx;
                $.ajax({
                    url: url,
                    type: "POST",
                    data: {
