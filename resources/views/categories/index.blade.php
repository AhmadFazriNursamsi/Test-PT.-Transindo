...
$haveaccessadd = Helpers::checkaccess('categories', 'add');
$haveaccessdelete = Helpers::checkaccess('categories', 'delete');
?>
...
<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight hetf2"><i class="fa fa-plus"></i>
            {{ __('Categories') }} <?php if($haveaccessadd): ?> 
                    ...
                    <div class="table-responsive">
                        ...
                        <table id="datastable" class="table text-start table-striped align-middle table-bordered table-hover mb-0">
                            <thead>
                                <tr>
                                    <td></td>
                                    <td><input type="text" class="form-control input-sm src_class_user" autocomplete="off" onkeyup="searcAjax(this)" placeholder="Nama Kategori" name="category_name"></td>
                                    <td>
                                ...
                                <tr>
                                    ...
                                    <th class="align-center">Nama Kategori</th>
                        ...
                        <div class="col-md-12">
                            <form id="smbtn" enctype="multipart/form-data"> 
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control inpt-cst-add mb-2" name="category_name" id="category_name" placeholder="Nama Kategori">
                                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    @if ($haveaccessadd) :
                        <span id="editvbtn" data-attid="" class="btn btn-success btn-sm"><i class="fa fa-edit"></i>Edit Category</span>
                    @endif
...
<script type="text/javascript">
    ...
    var url = "{{ asset('/api/categories/getdata') }}";
    // console.log(url);
    ...
    $(document).ready(function()
    {
        var table = $('#datastable').DataTable({
            ...
            createdRow: function(row, data, dataIndex, cells) {
                ...
                // console.log(data[1]); // Nama Kategori
...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    $("#btnAdd").click(function(){
        ...
        $('#active').val(1).change(); //
        $('#viewad').modal('show'); 
        $("#ModalLongTitle").html("Category Tambah"); // title MODAL CREATE
        $("#addvbtn").html('<i class="fa fa-plus"></i> Add Category');
    });
    // UNTUK MEMBUKA MODAL ADD
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    $("#smbtn").submit(function(e)
    {
        ...
        var url = "{{ asset('api/categories/insertdata') }}";
        if(id != '') // versi produk
            var url = "{{ asset('api/categories/updatedata') }}/" + id;
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    function showdetail(idx) 
    {
        ...
        $('#deletevbtn').html('<i class="fa fa-trash"></i>Delete Kategori');

        $('#undeletevbtn').attr('data-attid', idx);
        $('#undeletevbtn').html('<i class="fa fa-repeat"></i> Undelete Kategori');

        var table3 = document.querySelector("#table_detail tbody");
        table3.innerHTML= "";

        var url = "{{ asset('/api/categories/getdatabyid') }}/" + idx; // Table View Details
        // console.log(url);
        ...
        $.ajax({
            ...
            success: function(response) 
            {
                ...
                //     <td>'+data.category_name+'</td>\
                //     <td>\
                    //         if (data == '+data.active+' && item == '+1+'){\
                //             data = "<span style="color: #198754">active</span>";\
                //         }\
                //         if (data == '+data.active+' && item == '+0+'){\
                //             data = "<span style="color: #dc3545">non active</span>";\
                //         }\
                //     </td>\
                // </tr>';
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    {        
        ...
        $("#ModalLongTitle").html("Edit Kategori");
        clearInput("inpt-cst-add"); // pak heri

        $("#addvbtn").html('<i class="fa fa-edit"></i> Edit Kategori');
        $("#view").modal('hide');
        $('#viewad').modal('show');
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    function deleteyesshow() {
        ...
        Swal.fire({
            ...
            {
                var url = "{{ asset('/api/categories/delete') }}/" + idx;
                // console.log(url);
                $.ajax({
                    url: url,
                    type: "POST",
